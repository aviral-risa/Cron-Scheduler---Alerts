-- Astera Yesterday's Assigned Cases Still Unworked V1
--
-- Lists cases assigned on @report_date (IST) that remain master_auth_status = 'new'.
-- Intended for re-allocation: grouped per assignee in Slack formatter.
--
-- Parameters:
--   @report_date DATE — prior IST day (yesterday)
--   @org_id STRING
...
WHERE LOWER(TRIM(COALESCE(o.assigned_to_name, ''))) NOT IN ('', 'unassigned', 'risa agent')
  ...
  AND DATE(ca.assigned_at, 'Asia/Kolkata') = p.report_date
ORDER BY assigned_to, mrn;
