/**
 * Algolia Data Transformation Service
 *
 * Transforms Algolia order objects to OrderSnapshot format for backward compatibility
 */

import type { AlgoliaOrder } from '../types/algolia.types';
...
export class AlgoliaTransformService {
  /**
  ...
  transformOrders(algoliaOrders: AlgoliaOrder[]): OrderSnapshot[] {
    logger.info(`Transforming ${algoliaOrders.length} orders...`);
    ...
    if (errors.length > 0) {
      logger.warn(`Failed to transform ${errors.length} orders`);
      errors.forEach((err) => {
        logger.warn(`  Order ${err.index}: ${err.error}`);
      });
    }

    logger.success(`Transformation complete: ${snapshots.length}/${algoliaOrders.length} successful`);
    return snapshots;
  }

  /**
   * Validate that a transformed snapshot has all required fields
   */
  validateSnapshot(snapshot: OrderSnapshot): { valid: boolean; missingFields: string[] } {
